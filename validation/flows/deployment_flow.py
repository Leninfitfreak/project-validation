...
def run(page: Page, config: ValidationConfig, recorder: RunRecorder) -> None:
    ...
    if login_flows.jira_ui_available(config):
        ...
        try:
            ...
            if jira_ticket not in (page.text_content("body") or ""):
                login_flows.try_login_jira(page, config, long_timeout)
            capture_when_ready(
                page,
                config.screenshot_dir("deployment") / "jira-ticket-proof.png",
                require_no_loading=False,
                verify=lambda: deployment_ui_checks.jira_ticket_page(
                    page,
                    jira_ticket,
            ...
            recorder.add_step(
                StepResult(
                    ...
                    "Jira ticket proof",
                    "PASS",
                    "Real Jira ticket page captured in the browser with visible ticket details",
                    "screenshots/deployment/jira-ticket-proof.png",
                )
            )
        except Exception as exc:
            warnings.append(f"Direct Jira browser proof was not captured: {exc}")
            recorder.add_step(
                StepResult(
                    "DEP-001",
                    "deployment",
                    "Jira ticket proof",
                    "WARN",
                    "Real Jira UI proof could not be captured because a browser-authenticated Jira session was not available",
                )
            )
    else:
        warnings.append("Jira browser UI credentials or ticket URL template are not configured in project-validation.")
        recorder.add_step(
            StepResult(
                "DEP-001",
                "deployment",
                "Jira ticket proof",
                "WARN",
                "Real Jira UI proof is not configured in project-validation; no synthetic fallback was used as primary evidence",
            )
        )
    ...
    recorder.add_step(
        StepResult(
            ...
            "PASS",
            "Real GitHub Actions workflow run page captured with job summary visible",
    ...
    recorder.add_step(
        StepResult(
            ...
            "PASS",
            "Real GitHub job page captured with the self-hosted runner details visible",
    ...
    recorder.add_step(
        StepResult(
            ...
            "PASS",
            "Real GitHub workflow run page captured with the deployment-result artifact visible as primary browser proof",
    ...
    recorder.add_step(
        StepResult(
            ...
            "GitOps commit proof",
            "PASS",
            "Real public GitHub commit page shows the leninkart-infra revision and changed values file",
    ...
    try:
        ...
        argocd_page.goto(
            f"{config.env['ARGOCD_URL'].rstrip('/')}/applications/{target['argocd_app']}",
        ...
        capture_when_ready(
            ...
            require_no_loading=False,
            verify=lambda: deployment_ui_checks.argocd_deployment_detail(
                argocd_page,
                target["argocd_app"],
                payload["gitops_commit"],
                long_timeout,
            ),
            retries=waits["retry_count"],
            retry_wait_ms=waits["retry_sleep_ms"],
    ...
    recorder.add_step(
        StepResult(
            ...
            "PASS",
            "Real ArgoCD application page shows Synced and Healthy on the expected revision",
    ...
    recorder.add_step(
        StepResult(
            ...
            "PASS",
            "Real browser screenshot confirms the deployed LeninKart application is reachable",
