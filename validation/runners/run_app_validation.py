...
from validation.flows import app_auth_flow, order_flow, product_flow
...
def main() -> int:
    ...
    with BrowserHarness(config) as browser:
        ...
        app_auth_flow.run(page, config, recorder)
        products = product_flow.run(page, config, recorder)
        order_flow.run(page, config, recorder, products)
        page.close()
